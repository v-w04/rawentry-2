...
function _setPantalla(p){
  ...
  if(act)    act.classList.remove('active');/* RAW Entry — Logros v.5.076
   Activity Check → 5 columnas simultáneas
   Sims Needs → barras estilo Sims, colores dinámicos
*/
// ══════════════════════════════════════════
...
function _colActividad(titulo, color, colId, innerHtml, counter){
  ...
  if(counter !== undefined){
    ...
    cntHtml = '<span style="font-size:10px;font-weight:700;color:'+cColor+';margin-left:4px;font-variant-numeric:tabular-nums">'+
      counter.done+'/'+counter.total+'</span>';
  }
